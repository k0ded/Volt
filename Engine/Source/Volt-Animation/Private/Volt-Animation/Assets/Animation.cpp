...
namespace Volt
{
	...
	const Vector<glm::mat4> Animation::Sample(float samplePercent, Ref<Skeleton> skeleton, bool looping)
	{
		...
		{
			VT_LOG(Error, "Sample percent has to be a value between 0 and 1");
			return {};
		}
		...
		if (frameCount == 0)
		{
			VT_LOG(Error, "Tried to sample animation with no frames");
			return {};
		}
	...
	void Animation::BlendPoseWith(Pose& target, const Pose& poseToBlendWith, float blendFactor)
	{
		...
		{
			VT_LOG(Error, "Cannot blend poses with different joint counts");
			return;
		}
