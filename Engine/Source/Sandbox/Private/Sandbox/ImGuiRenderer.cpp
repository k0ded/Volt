...
void Sandbox::DrawSaveSceneAs(Volt::AssetHandle handle)
{
	UI::PushID();
	...
	if (ImGui::Button("Save"))
	{
		...
		Volt::AssetManager::SaveMemoryAssetToDirectory(handle, m_saveSceneData.destinationPath);

		//todo_fabian: this should happen automatically
		DirtyAssetsManager::Get().MarkAssetNotDirty(handle);

		m_dirtyAssetExternalSaveData.SceneSavedAs = true;
	}
}
