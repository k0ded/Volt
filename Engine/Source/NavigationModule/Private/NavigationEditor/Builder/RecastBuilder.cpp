							   ...
							   const int maxTiles)
{
	...
	{
		VT_LOG(Error, "buildTile: Input mesh is not specified.");
		return 0;
	}
	...
	if (!rc.triareas)
	{
		VT_LOG(Error, "buildNavigation: Out of memory 'm_triareas' ({0}).", chunkyMesh->maxTrisPerChunk);
		return 0;
	}
...
Ref<Volt::AI::NavMesh> RecastBuilder::BuildSingleNavMesh()
{
	...
	{
		VT_LOG(Error, "buildNavigation: Input mesh is not specified.");
		return nullptr;
	}
	...
	VT_LOG(Info, "Building navigation:");
	VT_LOG(Info, " - {0} x {1} cells", m_cfg.width, m_cfg.height);
	VT_LOG(Info, " - {0}K verts, {1}K tris", nverts / 1000.0f, ntris / 1000.0f);
	...
	if (!m_triareas)
	{
		VT_LOG(Error, "buildNavigation: Out of memory 'm_triareas' ({0}).", ntris);
		return nullptr;
	}
	...
	if (!m_dmesh)
	{
		VT_LOG(Error, "buildNavigation: Out of memory 'pmdtl'.");
		return nullptr;
	}
	...
	if (m_cfg.maxVertsPerPoly <= DT_VERTS_PER_POLYGON)
	{
		...
		{
			VT_LOG(Error, "Could not build Detour navmesh.");
			return nullptr;
		}
		...
		{
			dtFree(navData);
			VT_LOG(Error, "Could not create Detour navmesh");
			return nullptr;
		}
		...
		{
			dtFree(navData);
			VT_LOG(Error, "Could not init Detour navmesh");
			return nullptr;
		}
	...
	VT_LOG(Info, ">> Detailed Polymesh: {0} vertices, {1} triangles", m_dmesh->nverts, m_dmesh->ntris);
	VT_LOG(Info, ">> Polymesh: {0} vertices, {1} polygons", m_pmesh->nverts, m_pmesh->npolys);

	m_totalBuildTimeMs = m_ctx->getAccumulatedTime(RC_TIMER_TOTAL) / 1000.0f;
	VT_LOG(Info, ">> NavMesh finished building in {0}ms", m_totalBuildTimeMs);
...
Ref<Volt::AI::NavMesh> RecastBuilder::BuildTiledNavMesh()
{
	if (!m_geom || !m_geom->getVertCount())
	{
		VT_LOG(Error, "buildTiledNavigation: Input mesh is not specified.");
		return nullptr;
	}
	...
	if (!tileCache)
	{
		VT_LOG(Error, "buildTiledNavigation: Could not allocate tile cache.");
		return nullptr;
	}
	...
	if (!navMesh)
	{
		VT_LOG(Error, "buildTiledNavigation: Could not allocate navmesh.");
		return nullptr;
	}
	...
	if (dtStatusFailed(status))
	{
		VT_LOG(Error, "buildTiledNavigation: Could not init navmesh.");
		return nullptr;
	}
	...
	// Show performance stats.
	VT_LOG(Info, ">> Layers: {0}", m_cacheLayerCount);
	VT_LOG(Info, ">> Layers (per tile) {0}", (float)m_cacheLayerCount / (float)gridSize);
	VT_LOG(Info, ">> Memory: {0} kB / {1} kB ({2})", m_cacheCompressedSize / 1024.0f, m_cacheRawSize / 1024.0f, compressionRatio * 100.0f);
	VT_LOG(Info, ">> Build Peak Mem Usage: {0} kB", m_cacheBuildMemUsage / 1024.0f);
	VT_LOG(Info, ">> NavMesh finished building in {0}ms", m_cacheBuildTimeMs);
