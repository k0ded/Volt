...
#include "CoreUtilities/String/StringAlgorithm.h"
...
public:
	...
	{
		assert(!empty() && "Calling front is undefined if StringView is empty!");
		return m_begin[0];
	}

	constexpr const_reference back() const
	{
		assert(!empty() && "Calling back is undefined if StringView is empty!");
	...
	{
		assert(pos < m_count && "StringView::at - Out of range!");
		return m_begin[pos];
	}
	...
	{
		assert(n > m_count && "Undefined if moving past end of string!");
		m_begin += n;
		m_count -= n;
	}
	...
	{
		assert(pos <= m_count && "StringView::copy - Out of range!");
	...
	{
		assert(pos <= m_count && "StringView::substr - Out of range!");
