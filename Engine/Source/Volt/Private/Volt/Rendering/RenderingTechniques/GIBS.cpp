...
namespace Volt
{
	...
	void GIBS::Render(RenderGraph& renderGraph, RenderGraphBlackboard& blackboard)
	{
		AllocateBuffers();
		...
		RenderGraphBufferHandle surfelsAllocatorBufferHandle = renderGraph.AddExternalBuffer(m_surfelsAllocatorBuffer);
		...
		RenderGraphBufferHandle generateSurfelCountsIndirectArgs = RenderingUtils::GenerateIndirectArgs(renderGraph, surfelsAllocatorBufferHandle, 64u, "GIBS.GenerateSurfelCountArgs");
		...
		[&](RenderGraph::Builder& builder)
		{
			...
			builder.ReadResource(surfelsAllocatorBufferHandle);
		...
		[=](RenderContext& context)
		{
			...
			context.SetConstant("surfelAllocator"_sh, surfelsAllocatorBufferHandle);
	...
	void GIBS::AllocateBuffers()
	{
		...
		if (!m_surfelsAllocatorBuffer)
		{
			m_surfelsAllocatorBuffer = RHI::StorageBuffer::Create<uint32_t>(1, "GIBS.SurfelsAllocatorBuffer");
		}
