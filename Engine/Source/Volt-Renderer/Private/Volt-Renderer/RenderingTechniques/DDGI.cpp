...
namespace Volt
{
	...
	void DDGI::Render(RenderGraph& renderGraph, RenderGraphBlackboard& blackboard, Ref<RenderScene> renderScene)
	{
		...
		{
			auto pipeline = ShaderMap::GetComputePipeline("DDGIUpdateProbes");
			...
			context.SetConstant("probeSpacing"_sh, s_ddgiProbeSpacing.GetValue());
			context.SetConstant("probeGridSize"_sh, static_cast<uint32_t>(s_ddgiProbeGridSize.GetValue()));
			context.SetConstant("probeResolution"_sh, static_cast<uint32_t>(s_ddgiProbeResolution.GetValue()));
