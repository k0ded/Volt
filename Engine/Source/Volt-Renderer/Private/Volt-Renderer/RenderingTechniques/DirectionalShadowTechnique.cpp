...
namespace Volt
{
	...
	DirectionalShadowData DirectionalShadowTechnique::Execute(Ref<Camera> camera, Ref<RenderScene> renderScene)
	{
		...
		const glm::mat4 view = glm::lookAtLH(glm::vec3(0.f) - glm::vec3(lightInfo.direction) * 1000.f, glm::vec3(0.f), glm::vec3(0.f, 1.f, 0.f));
		...
		DirectionalShadowData dirShadowData{};
		{
			...
			imageDesc.layers = DirectionalLightShadowUniformBuffer::CASCADE_COUNT;
		...
		for (uint32_t i = 0; i < DirectionalLightShadowUniformBuffer::CASCADE_COUNT; i++)
		{
			...
			[=](RenderContext& context)
			{
				...
				info.renderingInfo.layerCount = DirectionalLightShadowUniformBuffer::CASCADE_COUNT;
