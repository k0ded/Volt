...
namespace Volt
{
	...
	public:
		...
		IntRef<RHI::Shader> GetShader(const T::PermutationVector& permutationVector)
		{
			...
			IntRef<RHI::Shader> shader = GetShaderInternal(typeIndex, permutationIndex, isDefaultShader);
		...
		template<typename T>
		IntRef<RHI::Shader> GetShader()
		{
			constexpr TypeTraits::TypeIndex typeIndex = TypeTraits::TypeIndex::FromType<T>();

			bool isDefaultShader = false;
			IntRef<RHI::Shader> shader = GetShaderInternal(typeIndex, 0, isDefaultShader);
	...
	private:
		struct ShaderBucket
		{
			Map<size_t, IntRef<RHI::Shader>> permutations;
			IntRef<RHI::Shader> defaultShader;
		};

		VTR_API IntRef<RHI::Shader> GetShaderInternal(TypeTraits::TypeIndex shaderType, size_t permutationIndex, bool& isDefaultShader);
		VTR_API IntRef<RHI::Shader> CompileShaderPermutation(TypeTraits::TypeIndex shaderType, size_t permutationIndex, RHI::ShaderPermutationConfig&& permutationConfig);
