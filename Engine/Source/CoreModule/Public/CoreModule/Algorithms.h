...
namespace Volt::Algo
{
	...
	extern VTC_API uint32_t GetThreadCountFromIterationCount(uint32_t iterationCount);

	template<typename T>
	extern Vector<uint32_t> ElementCountPrefixSum(const Vector<Vector<T>>& elements)
	{
