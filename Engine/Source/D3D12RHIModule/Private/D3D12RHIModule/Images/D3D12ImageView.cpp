...
namespace Volt::RHI
{
	...
	D3D12ImageView::D3D12ImageView(const ImageViewDesc specification) 
	...
	void D3D12ImageView::CreateRTVDSV()
	{
		...
		auto* resourcePtr = m_desc.image->GetHandle<ID3D12Resource*>();
		...
		if (Utility::IsDepthFormat(m_format))
		{
			...
			else if (m_desc.viewType == ImageViewType::View2DArray)
			{
				...
				viewDesc.Texture2DArray.ArraySize = m_desc.layerCount;
	...
	void D3D12ImageView::CreateSRV()
	{
		...
		if (m_desc.viewType == ImageViewType::View2D)
		{
			...
			viewDesc.Texture2D.MipLevels = m_desc.mipCount;
		...
		else if (m_desc.viewType == ImageViewType::View2DArray)
		{
			...
			viewDesc.Texture2DArray.MipLevels = m_desc.mipCount;
			viewDesc.Texture2DArray.ArraySize = m_desc.layerCount;
			viewDesc.Texture2DArray.FirstArraySlice = m_desc.baseArrayLayer;
		...
		else if (m_desc.viewType == ImageViewType::ViewCube)
		{
			...
			viewDesc.TextureCube.MipLevels = m_desc.mipCount;
