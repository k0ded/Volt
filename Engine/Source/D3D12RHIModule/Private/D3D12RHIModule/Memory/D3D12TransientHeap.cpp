...
namespace Volt::RHI
{
		...
		: m_createInfo(info)
	{
		if ((info.flags & TransientHeapFlags::AllowBuffers) != TransientHeapFlags::None)
		{
			...
			m_bufferAllocationArena.Reserve(256);
		}
		else if ((info.flags & TransientHeapFlags::AllowTextures) != TransientHeapFlags::None || (info.flags & TransientHeapFlags::AllowRenderTargets) != TransientHeapFlags::None)
		{
			...
			m_imageAllocationArena.Reserve(256);
		}
	}
	...
	Handle<Allocation> D3D12TransientHeap::CreateBuffer(const TransientBufferCreateInfo& createInfo, const std::string& name)
	{
		...
		bufferAlloc->m_memoryRequirements.size = createInfo.size;
	...
	Handle<Allocation> D3D12TransientHeap::CreateImage(const TransientImageCreateInfo& createInfo, const std::string& name)
	{
		...
		imageAlloc->m_memoryRequirements.size;
