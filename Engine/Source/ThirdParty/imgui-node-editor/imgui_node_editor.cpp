...
void ed::Link::Draw(ImDrawList* drawList, ImU32 color, float extraThickness) const
{
	...
	{
		case ax::NodeEditor::ArrowLocation::None:
		{
		...
		case ax::NodeEditor::ArrowLocation::Start:
		{
			ImDrawList_AddBezierWithArrows(drawList, curve, m_Thickness + extraThickness,
					m_StartPin ? m_StartPin->m_ArrowSize + extraThickness : 0.0f,
					m_StartPin ? m_StartPin->m_ArrowWidth + extraThickness : 0.0f,
					0.0f,
					0.0f,
					true, color, 1.0f);
			break;
		...
		case ax::NodeEditor::ArrowLocation::End:
		{
			ImDrawList_AddBezierWithArrows(drawList, curve, m_Thickness + extraThickness,
					0.0f,
					0.0f,
					m_EndPin ? m_EndPin->m_ArrowSize + extraThickness : 0.0f,
					m_EndPin ? m_EndPin->m_ArrowWidth + extraThickness : 0.0f,
					true, color, 1.0f);
			break;
		}

		case ax::NodeEditor::ArrowLocation::StartEnd:
		{
			ImDrawList_AddBezierWithArrows(drawList, curve, m_Thickness + extraThickness,
					m_StartPin ? m_StartPin->m_ArrowSize + extraThickness : 0.0f,
					m_StartPin ? m_StartPin->m_ArrowWidth + extraThickness : 0.0f,
...
void ed::Link::UpdateEndpoints()
{
	...
	m_Start = line.A + m_Offset;
	m_End = line.B + m_Offset;
}
...
{
	// #VOLT: Changed because of crash on destruction of editor
	//if (m_IsInitialized)
	//    SaveSettings();
...
bool ed::EditorContext::DoLink(LinkId id, PinId startPinId, PinId endPinId, const ImVec2& offset, ArrowLocation arrowLocation, ImU32 color, float thickness)
{
	auto startPin = FindPin(startPinId);
	auto endPin = FindPin(endPinId);
	...
	link->m_Thickness = thickness;
	link->m_IsLive = true;
	link->m_Offset = offset;
	link->m_ArrowLocation = arrowLocation;
