...
namespace ImGui
{
    ...
    int EditorPopup(const char* label, float P[5])
    {
        ...
        hovered |= 0 != ItemHoverable(ImRect(bb.Min, bb.Min + ImVec2(avail, dim)), id, ImGuiItemFlags_Default_);
        ...
        // control points: 2 lines and 2 circles
        {
            ...
            if (distance[selected] < (3 * GRAB_RADIUS * 3 * GRAB_RADIUS))
            {
        ...
        ImVec4 green(0.00f, 1.00f, 0.f, luma), blue(0.00f, 0.00f, 1.00f, luma);
        ImVec2 p1 = ImVec2(P[0], 1 - P[1]) * (bb.Max - bb.Min) + bb.Min;
        ImVec2 p2 = ImVec2(P[2], 1 - P[3]) * (bb.Max - bb.Min) + bb.Min;
        DrawList->AddLine(ImVec2(bb.Min.x, bb.Max.y), p1, ImColor(white), LINE_WIDTH);
        DrawList->AddLine(ImVec2(bb.Max.x, bb.Min.y), p2, ImColor(white), LINE_WIDTH);
        DrawList->AddCircleFilled(p1, GRAB_RADIUS, ImColor(white));
        DrawList->AddCircleFilled(p1, GRAB_RADIUS - GRAB_BORDER, ImColor(green));
    ...
    int Bezier(const char* label, float P[5])
    {
        ...
        // Draw graph preview instead of button

        if (ImGui::Button("Open Editor"))
        {
            ImGui::OpenPopup("!BezierEditor");
        }
        ImGui::SameLine();
        ...
        if (ImGui::BeginPopup("!BezierEditor"))
        {
            changed = EditorPopup(label, P);
            ImGui::EndPopup();
        }
