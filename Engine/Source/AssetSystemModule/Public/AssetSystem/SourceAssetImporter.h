...
namespace Volt
{
	...
	public:
		...
		Vector<AssetReference<Asset_New>> Import(const std::filesystem::path& filepath, const ConfigType& config, const SourceAssetUserImportData& userData = {})
		{
			return ImportInternal(filepath, reinterpret_cast<const void*>(&config), userData);
		}

	protected:
		virtual Vector<AssetReference<Asset_New>> ImportInternal(const std::filesystem::path& filepath, const void* config, const SourceAssetUserImportData& userData) const = 0;
	};
}
