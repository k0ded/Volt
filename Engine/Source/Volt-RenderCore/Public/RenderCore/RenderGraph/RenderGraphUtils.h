...
namespace Volt
{
	...
	*/
	...
	namespace ComputeShaderUtils
	{
		...
		void AddPass(RenderGraph& renderGraph, const String& passName, IntRef<RHI::Shader> shader, const typename ShaderType::Parameters* passParameters, RenderGraphPassFlags flags, const glm::uvec3& dispatchSize)
		{
		...
		template<typename ShaderType>
		void AddPass(RenderGraph& renderGraph, const String& passName, IntRef<RHI::Shader> shader, const typename ShaderType::Parameters* passParameters, const glm::uvec3& dispatchSize)
		{
			AddPass<ShaderType>(renderGraph, passName, shader, passParameters, RenderGraphPassFlags::None, dispatchSize);
		}

		template<typename ShaderType>
		void AddPass(RenderGraph& renderGraph, const String& passName, IntRef<RHI::Shader> shader, const typename ShaderType::Parameters* passParameters, RenderGraphPassFlags flags, RGBufferRef indirectArgsBuffer, uint64_t argsOffset)
		{
			renderGraph.AddPass(passName,
				RenderGraphPassFlags::Compute | flags,
				passParameters,
		...
		template<typename ShaderType>
		void AddPass(RenderGraph& renderGraph, const String& passName, IntRef<RHI::Shader> shader, const typename ShaderType::Parameters* passParameters, RGBufferRef indirectArgsBuffer, uint64_t argsOffset)
		{
