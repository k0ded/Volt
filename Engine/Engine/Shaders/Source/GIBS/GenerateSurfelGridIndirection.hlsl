...
void GenerateSurfelGridIndirection(uint dispatchThreadID : SV_DispatchThreadID)
{
    ...
    const uint surfelCellStartOffset = constants.surfelCellPrefixSums.Load(surfelCellIndex);

    uint index;
    InterlockedAdd(constants.surfelCellCounts, surfelCellIndex, 1, index);

    constants.surfelCellIndirections.Store(surfelCellStartOffset + index, dispatchThreadID);
