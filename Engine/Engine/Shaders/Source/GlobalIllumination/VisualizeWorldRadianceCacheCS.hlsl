...
[numthreads(8, 8, 1)]
void VisualizeWorldRadianceCacheCS(uint2 DispatchThreadID : SV_DispatchThreadID)
{
	...
	if (pixelDepth > 0.f)
	{
		...
		{
			RWSceneColor[DispatchThreadID] = float4(UnpackRGBE(WorldRadianceCacheCellCache[hashIndex]), 1.f);
		}
		else
		{
